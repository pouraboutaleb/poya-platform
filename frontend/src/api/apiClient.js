...
export const warehouseRequests = {
  getAll: async () => {
    const response = await apiClient.get('/api/v1/warehouse/warehouse-requests');
    return response.data;
  },
  
  getById: async (id) => {
    const response = await apiClient.get(`/api/v1/warehouse/warehouse-requests/${id}`);
    return response.data;
  },
  
  create: async (requestData) => {
    const response = await apiClient.post('/api/v1/warehouse/warehouse-requests', requestData);
    return response.data;
  },
  
  update: async (id, requestData) => {
    const response = await apiClient.put(`/api/v1/warehouse/warehouse-requests/${id}`, requestData);
    return response.data;
  }
};
